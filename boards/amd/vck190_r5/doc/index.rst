...
Xilinx VCK190 Development Board RPU Cortex-R5
############################################

Overview
********
This configuration provides support for the RPU, real-time processing unit on Xilinx
VCK190 development board, it can operate as following:
...
below points, betweem the options, to the vck190 target:

https://github.com/OpenAMP/openamp-ci-builds/releases/tag/v2022.12

Select the option ``xilinx-vck190.tar.gz``, and just decompress it to the target rootfs
partition of user's SD card:

.. code-block:: console

        $ sudo mount /dev/<user-sd> /media/rootfs
        $ sudo tar -C /media/rootfs -xzf xilinx-vck190.tar.gz
