#define LOG_MODULE_NAME eth_xilinx_axienet_mdio
...
#define XILINX_AXIENET_MDIO_SETUP_REG_MDIO_DISABLE_MASK       0x0
#define XILINX_AXIENET_MDIO_SETUP_REG_MDIO_ENABLE_MASK        (1 << 6)
#define XILINX_AXIENET_MDIO_SETUP_REG_MDIO_CLOCK_DIVIDER_MASK ((1 << 6) - 1)
#define XILINX_AXIENET_MDIO_CONTROL_REG_OFFSET                0x00000504
#define XILINX_AXIENET_MDIO_CONTROL_REG_MASK_READY            (1 << 7)
...
#define XILINX_AXIENET_MDIO_CONTROL_REG_MASK_READ             0x00008000
#define XILINX_AXIENET_MDIO_CONTROL_REG_MASK_WRITE            0x00004000
#define XILINX_AXIENET_MDIO_CONTROL_REG_MASK_INITIATE         (1 << 11)
#define XILINX_AXIENET_MDIO_WRITE_DATA_REG_OFFSET             0x00000508
#define XILINX_AXIENET_MDIO_READ_DATA_REG_OFFSET              0x0000050C
#define XILINX_AXIENET_MDIO_READ_DATA_REG_DATA_MASK           0x0000ffff
...
#define MDIO_TARGET_FREQUENCY_HZ 2500000 /* 2.5 MHz, i.e., max MDIO clock according to IEEE spec */
#define MDIO_BUSY_WAIT_USEC      1
#define MDIO_BUSY_WAIT_MAX_USEC  10000

static void mdio_bus_wait_ready(const struct mdio_xilinx_axienet_config *config)
{
	int attempts = 0;

	while ((xilinx_axienet_read_mdio_register(config, XILINX_AXIENET_MDIO_CONTROL_REG_OFFSET) &
		XILINX_AXIENET_MDIO_CONTROL_REG_MASK_READY) == 0) {

		LOG_DBG("Waiting for MDIO to become ready!");
		k_busy_wait(MDIO_BUSY_WAIT_USEC);
		attempts += MDIO_BUSY_WAIT_USEC;
		if (attempts >= MDIO_BUSY_WAIT_MAX_USEC) {
			LOG_ERR("Exceeded max wait time for busy waiting!");
			k_panic();
		}
	}
}
...
static void mdio_xilinx_axienet_bus_enable(const struct device *dev)
{
	...
	clock_divider_full = (config->clock_frequency_hz / (MDIO_TARGET_FREQUENCY_HZ)) * 2;
...
#define XILINX_AXIENET_MDIO_INIT(inst)                                                             \
	...
	static struct mdio_xilinx_axienet_data mdio_xilinx_axienet_data##inst = {                  \
		.bus_enabled = false,                                                              \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(inst, NULL, NULL, &mdio_xilinx_axienet_data##inst,                   \
			      &mdio_xilinx_axienet_config##inst, POST_KERNEL,                      \
			      CONFIG_MDIO_INIT_PRIORITY, &mdio_xilinx_axienet_api);
